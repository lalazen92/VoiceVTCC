package vtcc.lalazen.com.demovtcc.speedtotextcyberspace.myservice.speech2text;
...
import vtcc.lalazen.com.demovtcc.speedtotextcyberspace.UtilsVoice;
import vtcc.lalazen.com.demovtcc.speedtotextcyberspace.audio.Recorder;
    ...
    protected CountDownLatch record() {
        ...
        try {
            ...
            while (recorder != null && (byteRead = recorder.read(byte_buff_short, 0, byte_buff_short.length)) != -1 && isRecording) {
                ...
                double sDataMax = 0;
                for (int i = 0; i < byte_buff_short.length; i++) {
                    if (Math.abs(byte_buff_short[i]) >= sDataMax) {
                        sDataMax = Math.abs(byte_buff_short[i]);
                    }
                    ;
                    // sDataString += "\n" + sDataMax;
                    Log.d("duy8k1", "sDataMax= " + sDataMax);
                    if (stopRecordListener != null) {
                        stopRecordListener.volumeChange((float) sDataMax);
                    }
                }
